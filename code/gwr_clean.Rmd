...
```{r}
# Plot the proportion of park area per neighborhood
ggplot() +
  geom_sf(data = census2016_sf, aes(fill = proportion_park_area)) +
  scale_fill_gradient(name = "Proportion of Park Area", low = "grey", high = "darkgreen") +
  labs(title = "Proportion of Park Area per Neighborhood")
```
...
# Overlay libraries
libraries_plot <- toronto_map +
  geom_sf(data = libraries, color = "red", size = 1) +
  theme_void()

# Print the map
print(libraries_plot)
```
...
equation6 <- num_crimes ~ unemployed + proportion_park_area + commute_public_transport + population_density
# List of equations
equations <- list(equation1, equation2, equation3, equation4, equation5, equation6)
```
