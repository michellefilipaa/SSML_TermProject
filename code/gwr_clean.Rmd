...
```{r}
# load in the geojson of Toronto
...
# 2016
census_path <- file.path(project_directory, "data", "census2016.csv")
census <- read.csv(census_path, row.names = 1)
...
```{r}
# compute the area of each neighbourhood
toronto$area <- st_area(toronto)
```

```{r}
# calculate the popualtion density
census$population_density <- census$total_population/toronto$area
```

## Add Toronto Geometries to Census 2016

Currently, the census file only has the name of the neighbourhoods but no geometries. We need to add the geometries from toronto.geojson to the census file to perform the analysis.
...
```{r}
# load in the geojson of parks in Toronto
...
```{r}
# Order by neighbourhoods and add to the census file
parks <- parks[order(parks$neighbourhood), ]
census_sf <- census_sf %>%
...
```{r}
# load in the geojson of libraries in Toronto
...
```{r}
# order by neighbourhood and add to census
libraries <- libraries[order(libraries$neighbourhood), ]
census_sf <- census_sf %>%
...
```{r}
# load in the geojson of police facilities in Toronto
...
```{r}
# load in the geojson of subway stops in Toronto
...
```{r}
# load in the geojson of Red light cameras in Toronto
...
```{r}
# add geometries to cameras data
...
```{r}
# add the neighbourhoods with no cameras to the data
cameras <- cameras[order(cameras$neighbourhood), ]
cameras <- rbind(cameras, toronto_missing3)
```

```{r}
# add the camera data to census
census_sf <- census_sf %>%
...
gwr_analysis <- function(equation, data, i) {
    crimedata_sp <- as_Spatial(data)
    print(paste("---Equation", i, "---"))
    abw <- bw.gwr(equation, approach = "AIC", adaptive = TRUE, kernel = "gaussian", data = crimedata_sp)
    
    a.gwr <- gwr.basic(equation, adaptive = TRUE, kernel = "gaussian", bw = abw, data = crimedata_sp)
...
eq3 <- num_crimes ~ commute_public_transport + income_under_5000 + immigrants + pop_15_to_19 + no_certificate_diploma_degree

# only physical indicators
...
```{r}
for (i in seq_along(gwr_results_list)[1:10]) {
    print(paste("---Equation", i, "---"))
    gwr_result <- gwr_results_list[[i]]
